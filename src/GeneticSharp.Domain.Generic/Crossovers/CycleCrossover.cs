...
namespace GeneticSharp.Domain.Crossovers.Generic
{
    ...
    public class CycleCrossover<T> : CrossoverBase<T>
    {
        #region Constructors
        /// <summary>
        /// Initializes a new instance of the <see cref="GeneticSharp.Domain.Crossovers.Generic.CycleCrossover"/> class.
        /// </summary>
        ...
        private static void CopyCycleIndexPair(IList<int> cycle, 
            IList<T> fromParent1Genes, IChromosome<T> toOffspring1, 
            IList<T> fromParent2Genes, IChromosome<T> toOffspring2)
        {
        ...
        /// <param name="cycle">The cycle.</param>
        private void CreateCycle(IList<T> parent1Genes, IList<T> parent2Genes, int geneIndex, List<int> cycle)
        {
            if (!cycle.Contains(geneIndex))
            {
                var parent2Gene = parent2Genes[geneIndex];
                cycle.Add(geneIndex);
                var newGeneIndex = parent1Genes.Select((g, i) => new { g, Index = i })
                    .First(g => g.Equals(parent2Gene)).Index;

                if (geneIndex != newGeneIndex)
                {
                    CreateCycle(parent1Genes, parent2Genes, newGeneIndex, cycle);
                }
            }
        }
        #endregion
