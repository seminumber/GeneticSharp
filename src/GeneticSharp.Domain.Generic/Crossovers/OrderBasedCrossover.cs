...
namespace GeneticSharp.Domain.Crossovers.Generic
{
    ...
    public class OrderBasedCrossover<T> : CrossoverBase<T>
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="GeneticSharp.Domain.Crossovers.Generic.OrderBasedCrossover"/> class.
        /// </summary>
        ...
        protected virtual IChromosome<T> CreateChild(IChromosome<T> firstParent, IChromosome<T> secondParent, int[] swapIndexes)
        {
            ...
            var firstParentSwapGenes = firstParentGenes
                .Select((g, i) => new { Gene = g, Index = i })
                .Where((g) => secondParentSwapGenes.Any(s => 
                    EqualityComparer<T>.Default.Equals(s.Gene, g.Gene)))
                .ToArray();
