...
namespace GeneticSharp.Domain.Crossovers.Generic
{
    ...
    public sealed class AlternatingPositionCrossover<T> : CrossoverBase<T>
    {
        ...
        protected override IList<IChromosome<T>> PerformCross(IList<IChromosome<T>> parents)
        {
            ...
            {
                throw new CrossoverException<T>(this, "The Alternating-position (AP) can be only used with ordered chromosomes. The specified chromosome has repeated genes.");
            }
            ...
            return new List<IChromosome<T>> { child1, child2 };
        }

        private IChromosome<T> CreateChild(IChromosome<T> firstParent, IChromosome<T> secondParent)
        {
            var child = firstParent.CreateNew();
            var childGenes = new T[firstParent.Length];
        ...
        private void AddChildGene(IList<T> childGenes, ref int childGenesIndex, 
            T parentGene)
        {
