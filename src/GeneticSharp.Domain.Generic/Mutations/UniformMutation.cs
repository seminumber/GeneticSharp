...
namespace GeneticSharp.Domain.Mutations.Generic
{
    ...
    public class UniformMutation<T> : MutationBase<T>
    {
        ...
        #region Constructors
        /// <summary>
        /// Initializes a new instance of the <see cref="GeneticSharp.Domain.Mutations.Generic.UniformMutation"/> class.
        /// </summary>
        ...
        protected override void PerformMutate(IChromosome<T> chromosome, float probability)
        {
            ...
            for (int i = 0; i < m_mutableGenesIndexes.Length; i++)
            {
                ...
                {
                    throw new MutationException<T>(this, "The chromosome has no gene on index {0}. The chromosome genes length is {1}.".With(geneIndex, genesLength));
                }
